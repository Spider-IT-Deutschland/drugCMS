...
if (isset($_REQUEST["listsubmit"]))
{
	...
	{
		$c_restrict = true;
		$currentuser->setUserProperty("mydrugcms", "hidedonetasks", "true");
	} else {
		$c_restrict = false;
		$currentuser->setUserProperty("mydrugcms", "hidedonetasks", "false");
	}
} else {
	if ($currentuser->getUserProperty("mydrugcms", "hidedonetasks") == "true")
	{
		$c_restrict = true;	
	} else {
		$c_restrict = false;	
...
if ($action == "mydrugcms_tasks_delete")
{
...
$link = new cHTMLLink;
$link->setCLink("mydrugcms_tasks_edit", 4, "");
...
while ($todo = $todoitems->next())
{
	if ((($todo->getProperty("todo", "status") != "done") && ($c_restrict == true))||($c_restrict == '')){
	{
    	...
    	$delete->setCLink("mydrugcms_tasks", 4, "mydrugcms_tasks_delete");
