    ...
    foreach ($settings as $key => $types)
    {
    	foreach ($types as $type => $value)
    	{
            ...
            if (in_array($key.'_'.$type, $aManagedValues)) {
    			...
    			if ((strtolower($type) == 'pass') || (strtolower($type) == 'password') || (strpos(strtolower($type), '-password-') !== false) || (strpos(strtolower($type), '_password_') !== false) || (substr(strtolower($type), -9) == '-password') || (substr(strtolower($type), -9) == '_password') || (substr(strtolower($type), 0, 9) == 'password-') || (substr(strtolower($type), 0, 9) == 'password_')) {
