...
if ($oRGroup->virgin == false && $oRGroup->get("idclient") == $client && $oRGroup->get("idlang") == $lang)
{
	...
	$oForm->add(i18n("Recipients in group", $plugin_name), '<table border="0" cellspacing="0" cellpadding="0" width="100%">'.
	...
	$oForm->add(i18n("Add recipients", $plugin_name), '<table border="0" cellspacing="0" cellpadding="0" width="100%">'.
										$oOutsiderListOptionRow->render().
										$oOutsiderPager->render().
										'<tr><td>'.$oSelUser->render().'<br />'.i18n("Note: Hold &lt;Ctrl&gt; to<br>select multiple items.", $plugin_name).'</td></tr></table>');	
	unset ($oOutsiders);
