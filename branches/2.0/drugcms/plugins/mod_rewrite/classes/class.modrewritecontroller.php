...
class ModRewriteController extends ModRewriteBase
{
    ...
    /**
     * Incomming URL components
     *
     * @var array
     */
    private $_aIncommingUrlComp;
    ...
    public function __construct($incommingUrl)
    {
        ...
        $this->_aIncommingUrlComp = @parse_url($this->_sIncommingUrl);
        $this->_aParts            = array();
    }
    ...
    private function _setPathresolverSetting()
    {
        ...
        if ($this->_hasPartArrayItems() == false) {
            // no path item exists, there is nothing to check
            return;
        } elseif ($this->_isFrontContentUrl()) {
            // incomming url contains front_content.php 
            return;
        }
    ...
    private function _setIdart()
    {
        ...
        if ($this->_isFrontContentUrl()) {
            // incomming url contains front_content.php, there is nothing to do
            return;
        }
    ...
    /**
     * Checks, if incomming URL contains front_content.php
     *
     * @return  bool
     */
    private function _isFrontContentUrl() {
        static $comp;
        if (!isset($comp)) {
            $comp = @parse_url($this->_sIncommingUrl);
        }
        return (isset($comp['path']) && strpos($comp['path'], 'front_content.php') !== false);
    }
