    ...
    function _recNode2Array($domnode, $aMergeTags) {
        ...
        if ($domnode->node_type() == XML_ELEMENT_NODE) {
            ...
            if(!is_array($result)) {
                ...
                #$result = $this->dummy_html_entity_decode(htmlentities($domnode->get_content(), ENT_COMPAT, strtoupper($this->_getEncoding())));
                $result = html_entity_decode(htmlentities($domnode->get_content(), ENT_COMPAT, strtoupper($this->_getEncoding()), ENT_COMPAT, strtoupper($encoding[$lang]));
            }
