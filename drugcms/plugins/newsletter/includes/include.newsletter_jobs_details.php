...
if ($action == "news_job_run" && $perm->have_perm_area_action($area, $action) && is_numeric($_REQUEST["idnewsjob"])) {
	...
	if ($oJob->get("dispatch") == 1 && $oJob->get("sendcount") < $oJob->get("rcpcount")) {
		...
		if ($oJob->get("dispatch_delay") == 0) {
			...
			$oForm->add("", sprintf(i18n("Sending newsletter ... (chunk %s of %s, recipients: %s, sent: %s)", $plugin_name),
	...
	$aData = Array("0"   => i18n("-All-", $plugin_name),
				   "50"  => "50",
				   "100" => "100",
				   "250" => "250",
				   "500" => "500");
	...
	$oImgDelete->setAlt(i18n("Delete item", $plugin_name));
	$sImgDelete = $oImgDelete->render();
	unset ($oImgDelete);
	
	$iCount = 0;
	$aNewsType[]  = array(); // Performance
	$aNewsType[0] = i18n("Text only", $plugin_name);
	$aNewsType[1] = i18n("HTML/Text", $plugin_name);
	while ($oLog = $oLogs->next())
	{
		...
		{
			case "pending":
				$sStatus = i18n("Waiting for sending", $plugin_name);
				break;
			case "sending":
				$sStatus = i18n("Sending", $plugin_name);
				break;
			case "successful":
				$sStatus = i18n("Successful", $plugin_name);
				break;
			default:
				$sStatus = sprintf(i18n("Error: %s", $plugin_name), $oLog->get("status"));
		}
	...
	<tr class="text_medium" style="background-color: #ffffff;">
        ...
        sprintf(i18n("Go to page: %s", $plugin_name), $sBrowseLinks) . '</td>
    </tr>
</table>';
	...
	switch ($oJob->get("status"))
	{
		case 1:
			...
			break;
		case 2:
			$oForm->add(i18n("Status", $plugin_name),	sprintf(i18n("Sending (started: %s)", $plugin_name), 
										date($sDateFormat, strtotime($oJob->get("started")))));
			break;
		case 9:
			$oForm->add(i18n("Status", $plugin_name),	sprintf(i18n("Finished (started: %s, finished: %s)", $plugin_name),
										date($sDateFormat, strtotime($oJob->get("started"))),
