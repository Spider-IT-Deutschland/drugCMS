...
<script type="text/javascript">
    function drugcms_dbg_toggle(myItem) {
    ...
    function drugcms_dbg_hide(myItem) {
        var myItemObj = document.getElementById(myItem);
        if (myItemObj) {
            myItemObj.style.display = \'none\';
        }
...
<style type="text/css">
    #drugcmsDbgBox { position:fixed; z-index:100000; max-height: 90%; max-width: 90%; left:5px; top:5px; margin:0; border:1px solid #ccc;padding:5px; background-color:#f6f6f6; text-align: left;color: #000000; overflow: auto;}
    #drugcmsDbgBox a { text-decoration:none; color: #000000; margin-top: 6px; }
    ...
    #drugcmsDbgClose { padding-left:10px; }
</style>';
            $sHtml .= '
<div id="drugcmsDbgBox">
    <a href="javascript:void(0);" title="Toggle Debug Output" onclick="drugcms_dbg_toggle(\'dbg_item_block\');">Debug info</a> 
    <a id="drugcmsDbgClose" href="javascript:void(0);" title="Hide Debug Output" onclick="drugcms_dbg_hide(\'drugcmsDbgBox\');">(x)</a>
    <div id="dbg_item_block" style="display:none;">';
            ...
            foreach ($this->_aItems as $oItem) {
                ...
                $sHtml .=  '
        <a href="javascript:void(0);" title="Toggle Item" onclick="drugcms_dbg_toggle(\'dbg_item_'.$i.'\');">'.$sItemName.'</a>
    ...
    private function _prepareValue($mValue) {
        ...
        if (is_array($mValue)) {
            $sReturn .= $this->_arrayToString($mValue);
    ...
    private function _arrayToString($array, $depth = 0) {
        if (is_array($array)) {
            $string = "array(\n";
            $depth ++;
            foreach ($array as $key => $val) {
                $string .= str_repeat('    ', $depth) . $this->_quoteTypeWrap($key) . ' => ';
                if (is_array($val)) {
                    $string .= $this->_arrayToString($val, $depth) . ",\n";
                } else {
                    $string .= $this->_quoteTypeWrap($val).",\n";
                }
            }
            $depth--;
    ...
    private function _quoteTypeWrap($var) {
        switch (gettype($var)) {
            case 'string':
                return '"' . $var . '"';
                break;
