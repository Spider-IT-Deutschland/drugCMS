...
abstract class Item extends Contenido_ItemBaseAbstract
{
    ...
    public function loadBy($sField, $mValue, $bSafe = true)
    {
        ...
        $aRecordSet = null;
        if (($sField === $this->primaryKey) && ($sField != 'idmod')) { # idmod has a problem with the xhtml property
            $aRecordSet = self::$_oCache->getItem($mValue);
        } else {
            $aRecordSet = self::$_oCache->getItemByProperty($sField, $mValue);
        }
    ...
    public function loadByRecordSet(array $aRecordSet)
    {
        ...
        $this->virgin        = false;
        self::$_oCache->addItem($this->oldPrimaryKey, $this->values);
    }
