...
if ( is_numeric($idcat) && ($idcat >= 0)) {
        ...
        WHERE
			...
			if ((($perm->have_perm_area_action("con","con_makestart")) || ($perm->have_perm_area_action_item("con","con_makestart",$idcat))) && ($idcat != 0)) {
                if ($is_start) {
                    ...
                    $tmp_link = '<a href="'.$sess->url("main.php?area=con&amp;idcat=$idcat&amp;action=con_makestart&amp;idcatart=$idcatart&amp;frame=4&amp;is_start=0&amp;next=$next").'" title="'.i18n("Flag as start article").'"><img src="images/'.$imgsrc.'" border="0" title="'.i18n("Flag as start article").'" alt="'.i18n("Flag as start article").'" style="margin-left: 7px;"></a>';
                }
            } else {
                if ($is_start) {
  ...
  $tpl->set('s', 'CLICK_ROW_NOTIFICATION', i18n("with click select line for further treatment"));
   ...
   $cat_name .= (isset($sFistArticleName))?htmlspecialchars($sFistArticleName):'';
			$cat_name .= "</div>";
		} else {
			$cat_name = '';
		}
