...
function strNewTree($catname, $catalias = '', $bVisible = 0, $bPublic = 1, $iIdtplcfg = 0) {
    ...
    if (in_array(getEffectiveSetting('system', 'auto-generate-start-articles', 'false'), array('true', '1'))) {
        foreach ($a_languages as $tmp_lang) {
            $idart_new = conEditFirstTime($tmp_newid, array($tmp_newid), null, true, null, null, $tmp_lang, $catname, '', null, date('Y-m-d H:i:s'), date('Y-m-d H:i:s'), $auth->auth['uname'], ((in_array(getEffectiveSetting('system', 'auto-generated-start-articles-online', 'true'), array('true', '1'))) ? 1 : 0), '0000-00-00 00:00:00', '0000-00-00 00:00:00', 0, null);
            $oArt = new Article($idart_new, $client, $lang);
            $sql = 'UPDATE ' . $cfg['tab']['cat_lang'] . '
                    SET startidartlang=' . $oArt->getField('idartlang') . '
                    WHERE (idcat=' . $tmp_newid . ')';
            $db->query($sql);
            unset($oArt);
        }
    }
