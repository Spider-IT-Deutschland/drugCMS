...
function handleInlineEdit(iCatId) {
    ...
    var layer = window.document.getElementById('cat_inline_layer');
    if (iCatId > 0) {   
        //Get References for editlayer, row to edit and category table content
        var row = window.document.getElementById('cat_'+iCatId+'_row'); 
        var table = window.document.getElementById('category_list').getElementsByTagName("tbody")[0];
        ...
        if (layer && row && table) {
            ...
            if (layer.style.display == 'block' || layer.style.display == 'table-row') {
                ...
                window.document.getElementById('cat_'+window.document.renamecategory.idcat.value+'_image').src= con_images+'but_todo.gif';
            }
            
            if (window.document.renamecategory.idcat.value != iCatId || bResetted == false) {
                ...
                window.document.getElementById('cat_'+iCatId+'_image').src= con_images+'but_todo_off.gif';
                
                //Get needed informations from dataArray and fill editform, also check perms
                window.document.renamecategory.idcat.value = iCatId;
                window.document.renamecategory.newcategoryalias.value = dataArray[iCatId]['alias']; 
                
                if(dataArray[iCatId]['pName'] == 1) {
                    window.document.getElementById('cat_name').style.display = 'inline';
                    window.document.renamecategory.newcategoryname.value = dataArray[iCatId]['catn']; 
                } else {
                    window.document.getElementById('cat_name').style.display = 'none';
                }
                
                if(dataArray[iCatId]['pTplcfg'] == 1) {
                    window.document.getElementById('tpl_cfg').style.display = 'inline';
                    if (oldHrefTplcfg == '') {
                        oldHrefTplcfg = window.document.getElementById('tplcfg_href').href;
                    }
                    window.document.getElementById('tplcfg_href').href = oldHrefTplcfg+'&idcat='+iCatId+'&idtpl='+dataArray[iCatId]['idtplcfg'];
                } else {
                    window.document.getElementById('tpl_cfg').style.display = 'none';
                    window.document.getElementById('tplcfg_href').href = oldHrefTplcfg;
                }
            }
        }
    } else {
...
function showNewForm() { 
    ...
    var oHideEditLayer = window.document.getElementById('cat_new_layer_disable');
    var oHideEditLayerImage = window.document.getElementById('cat_new_layer_disable_image');
...
function hideNewForm() {
	...
	window.document.getElementById('new_tree_button').style.color = '#000000';
}
...
function changeVisible() {
    var image = window.document.getElementById('visible_image');
    var label = window.document.getElementById('visible_label');
    var input = window.document.getElementById('visible_input');
...
function changePublic() {
    var image = window.document.getElementById('public_image');
    var label = window.document.getElementById('public_label');
    var input = window.document.getElementById('public_input');
...
function refreshStatus(bCaller) {
    var input = window.document.getElementById('new_idcat');                
    var checkbox = window.document.getElementById('is_tree');
    var conAction = window.document.getElementById('cat_new_action');
...
function checkForm() {
    var input = window.document.getElementById('new_idcat');                
    var checkbox = window.document.getElementById('is_tree');
    var category = window.document.getElementById('cat_categoryname').value;
...
function showTemplateSelect() {
    window.document.getElementById('cat_new_layer_disable').style.display = 'block';
    window.document.getElementById('new_idcat').disabled = true;  
  
    var oCategoryLayer = window.document.getElementById('cat_set_template_layer');
    var pos = getElementPostion( window.document.getElementById('cat_category_select_button') );
    
    window.document.getElementById('cat_new_submit').disabled = true;
    window.document.getElementById('cat_new_submit').src = con_images+'but_ok_off.gif';
    window.document.getElementById('cat_new_cancel').src = con_images+'but_cancel_off.gif';
    
    var select = window.document.getElementById('cat_template_select');
...
function hideTemplateSelect(bSave) {
    window.document.getElementById('cat_new_layer_disable').style.display = 'none';
    if (!window.document.getElementById('is_tree').checked) {
        window.document.getElementById('new_idcat').disabled = false; 
    }
  
    var oCategoryLayer = window.document.getElementById('cat_set_template_layer');
    oCategoryLayer.style.display = 'none';
    
    window.document.getElementById('cat_new_submit').disabled = false;
    window.document.getElementById('cat_new_submit').src = con_images+'but_ok.gif';
    window.document.getElementById('cat_new_cancel').src = con_images+'but_cancel.gif';
    
    if (bSave) {
        window.document.getElementById('idtplcfg_input').value = window.document.getElementById('cat_template_select').value;
